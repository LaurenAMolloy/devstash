...
const collectionTypeIconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  snippet: Code2,
  prompt: Sparkles,
  command: Terminal,
  note: FileText,
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {recentCollections.map((col) => (
            <div
              key={col.id}
              className="rounded-lg border border-border bg-card p-4 hover:border-border/80 hover:bg-accent/30 transition-colors cursor-pointer space-y-2"
            >
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className="text-sm font-medium truncate">{col.name}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">{col.itemCount} items</p>
                </div>
                {col.isFavorite && (
              ...
              <div className="flex items-center gap-1.5">
                {col.types.map((type) => {
                  const Icon = collectionTypeIconMap[type] ?? File;
                  return <Icon key={type} className="h-3 w-3 text-muted-foreground/60" />;
                })}
              </div>
            </div>
          ))}
